...
QCLOUD = np.asarray(data_wolf['QCLOUD'][:, :, 1:-1, 1:-1])
...
for i in range(185):
    ...
    elev, azim = 17, -45 # 傾斜程度、旋轉角度
    ax.view_init(elev=elev, azim=azim)

    Tset = ax.contourf(lon, lat, T[i], zdir='z', offset=0, cmap=cm.RdBu_r, levels=np.arange(15, 33+0.5, 0.5))
    RAINset = ax.contourf(lon, lat, RAINNC[i], zdir='z', offset=400, cmap=cm.coolwarm, alpha=0.7)
    QCLOUDset1 = ax.contourf(lon, lat, QCLOUD[i, 5], zdir='z', offset=height_mean[5], cmap=cm.coolwarm, alpha=0.7)
    QCLOUDset2 = ax.contourf(lon, lat, QCLOUD[i, 7], zdir='z', offset=height_mean[7], cmap=cm.coolwarm, alpha=0.7)
